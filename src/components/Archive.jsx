...
const Archive = () => {
  ...
  return (
    <div className="w-[85vw] flex flex-col items-center gap-4 h-[100vh]  border-r border-[#adadad] max-[1200px]:w-full">
      <div
        className={`"  ${
          archiveEmails.length !== 0 ? "w-full flex" : " w-full flex"
        } flex items-center border-b border-[#adadad] py-2 w-[432px]  px-4 min-h-[54px] justify-between max-[1200px]:w-full"`}
      >
        ...
        {archiveEmails.length === 0 ? (
          ...
          archiveEmails.map((email, index) => (
            ...
            <div
              ...
              className={`w-full overflow-hidden cursor-pointer transition-all duration-300 border border-[#adadad] text-left rounded-lg py-2 px-3 flex flex-col gap-1 ${
          ...
          {openModal && <div className="w-screen h-screen bg-[rgba(0,0,0,0.5)] justify-center flex items-center z-50 fixed left-0 top-0 backdrop-blur-sm">
           ...
           <div className="text-left flex items-center justify-between ">
            <div className="flex flex-col gap-2">
             <h2 className="text-[18px] font-semibold">From: {selectedArchive.Name}</h2>
             <p className=""><span className="font-semibold">Topic:</span> {selectedArchive.Topic}</p>
             <p className=""><span className="font-semibold">Sender email:</span> {selectedArchive.Email}</p>
            </div>
