...
export default function Sent (){
  ...
  return (
    <>
    <div className="w-[85.4vw] flex flex-col items-center bg-[#f1f8fd] gap-4 h-[100vh]  border-r border-[#adadad] text-[#111] " id="mainDiv">
      <div className="flex items-center border-b border-[#adadad] w-full  px-4 min-h-[54px] justify-between max-[720px]:w-full">
        ...
        id="button"
        onClick={deleteSent} 
        className="p-2 text-[#ffffff] bg-[#111] hover:bg-stone-700 rounded-lg active:scale-95  mr-8"
        >Clear Sent
        </button>
      </div>
        ...
        {sentEmails.length > 0 ? (
          sentEmails.map((email, index) => (
           ...
           onClick={handleCloseModal}
            className="absolute top-5 right-5 leading-none flex items-center justify-center p-2 text-3xl rounded-full border-2 border-[#adadad] text-[#111] bg-[#fff] hover:bg-stone-400 active:scale-95 ">
