...
const Mail_detail = () => {
    ...
    const inboxData = useContext(NoteContext);
    const selectedIndex = 0;

    const email = inboxData[selectedIndex];
            ...
            <div className="h-[95px] p-4 border-b-2 border-[#adadad] flex justify-between ">
                <div className="flex gap-4">
                    ...
                    <div className="text-left">
                        <h2 className="text-[15px] font-semibold">{email.Name}</h2>
                        <p className="text-[14px]">{email.Topic}</p>
                        <p className="text-[14px]">{email.Email}</p>
                    </div>
                </div>
                <div className="text-[14px] text-[#7c7b7b] flex gap-1">
                    <p>{email.Date}</p>, <p>{email.Time}</p>
                </div>
            </div>
            <div className="p-4 text-left">
                <h1>{email.MailHeading}</h1>
                <p>{email.MailContent}</p>
                <br />
                <p>{email.MailContent2}</p>
                <br />
                <p>{email.MailContent3}</p>
                <br />
                <br />
                <p>{email.MailContent4}</p>
            </div>
        </div>
    );
};
